...
//import red.server.Client;
import com.girlkun.database.GirlkunDB;
//import red.server.Manager;
...
//implements Runnable
    ...
    public static String gettoptrieuhoi() {
        StringBuffer sb = new StringBuffer("");

        String SELECT_TOP_POWER = "SELECT name, toptrieuhoi FROM player ORDER BY toptrieuhoi DESC LIMIT 10;";
        PreparedStatement ps;
        ResultSet rs;
        try {
            Connection conn = GirlkunDB.getConnection();
            ...
            byte i = 1;
            while(rs.next()) {
                sb.append(i).append(".").append(rs.getString("name")).append(": ").append(rs.getString("toptrieuhoi")).append("\b");
                i++;
            }
            conn.close();
        } catch (Exception e) {
