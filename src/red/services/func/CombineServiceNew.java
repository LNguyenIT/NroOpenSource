...
import red.services.RewardService;
...
import red.team.server.ServerNotify;
...
public class CombineServiceNew {
    ...
    private static final int COST_DOI_VE_DOI_DO_HUY_DIET = 5000;
    private static final int COST_DAP_DO_KICH_HOAT = 5000;
    private static final int COST_DOI_MANH_KICH_HOAT = 5000;
    ...
    private static final int NANG_CAP_DO_HD = 19897;
    ...
    public static final int PHAN_RA_DO_HOI_LONG = 540;
    ...
    public static final int DOI_SPL = 749;
    public static final int DOI_ITEM_C2 = 769;
    public static final int CHUYEN_HOA = 779;
    ...
    private final Npc Noel;
    private static CombineServiceNew i;

    public CombineServiceNew() {
        this.Noel = NpcManager.getNpc(ConstNpc.noel);
    ...
    public void showInfoCombine(Player player, int[] index) {
        ...
        switch (player.combineNew.typeCombine) {
            ...
            case MO_CHI_SO_CAI_TRANG:
                if (player.combineNew.itemsCombine.size() == 0) {
                    this.kiLan.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                ...
                if (player.combineNew.itemsCombine.size() == 2) {
                    if (player.combineNew.itemsCombine.stream().filter(
                            item -> item.isNotNullItem() && (item.template.type == 5))
                            .count() < 1) {
                        this.kiLan.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Thiếu cải trang", "Đóng");
                        return;
                    }
                    if (player.combineNew.itemsCombine.stream()
                            .filter(item -> item.isNotNullItem() && item.template.id == 1237)
                            .count() < 1) {
                        this.kiLan.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Thiếu lồng đèn", "Đóng");
                        return;
                    }

                    String npcSay = "Anh có muốn mở chỉ số cải trang!";

                    this.kiLan.createOtherMenu(player, ConstNpc.MENU_START_COMBINE,
                            npcSay, "Nâng cấp", "Từ chối");
                } else {
                    if (player.combineNew.itemsCombine.size() > 2) {
                        this.kiLan.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Nguyên liệu không phù hợp",
                                "Đóng");
                        return;
                    }
                    this.kiLan.createOtherMenu(player, ConstNpc.IGNORE_MENU,
            ...
            case NANG_CAP_CHAN_MENH:
                ...
                break;
            case PHAN_RA_DO_HOI_LONG:
                if (player.combineNew.itemsCombine.size() == 0) {
                    this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                            "Trả đồ hôi lông >3s - nhận vàng - và cút đi!",
                            "Đóng");
                    return;
                }
                int totalStar = 0;
                ...
                if (player.combineNew.itemsCombine.size() > 0) {
                    ...
                    if (player.combineNew.itemsCombine.size() != totalItem) {
                        String npcSay = "|2|Lượng đồ không khớp ?\n|7|";
                        this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                                npcSay, "Đóng");
                        return;
                    }
            ...
            case NANG_CAP_GAY_THIEN_SU:
                if (player.combineNew.itemsCombine.size() == 0) {
                    this.npsthiensu64.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                            "Hãy đưa ta 1 món gậy thiên sứ 20 đá nâng cấp 5, 50k Thỏi Vàng 2",
                            "Đóng");
                    return;
                }
            ...
            case CHE_TAO_TRANG_BI_TS:
                ...
                if (player.combineNew.itemsCombine.size() >= 2 && player.combineNew.itemsCombine.size() < 5) {
                    ...
                    npcSay += "|7|Phí nâng cấp: 500 ruby";
                    if (player.inventory.ruby < 5000) {
                        this.npsthiensu64.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Bạn không đủ ruby", "Đóng");
                        return;
                    }
                    this.npsthiensu64.createOtherMenu(player, ConstNpc.MENU_START_COMBINE,
                            npcSay, "Nâng cấp\n500 ruby", "Từ chối");
                ...
                if (player.combineNew.itemsCombine.size() == 2) {
                    ...
                    if (trangBi != null && vesc != null) {
                        ...
                        if (star + starEmpty < 8) {
                            ...
                            this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                                    "Tổng số sao đồ phải bé hơn 8", "Đóng");
                        }

                    } else {
                        this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                                "Thiếu nguyên liệu", "Đóng");
                    }
                } else {
                    this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                            "Cần 1 trang bị bất kì và 1 vé siêu cấp", "Đóng");
                }
                break;
            case NHAP_NGOC_RONG:
            ...
            case NANG_CAP_DO_TS:
                if (player.combineNew.itemsCombine.size() == 0) {
                    this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                            "1 công thức, 1 đá nâng cấp, 99 mảnh thiên sứ ", "Đóng");
                    return;
                }
                if (player.combineNew.itemsCombine.size() == 4) {
                    if (player.combineNew.itemsCombine.stream()
            ...
            case NANG_CAP_DO_HD:
                ...
                if (player.combineNew.itemsCombine.size() == 4) {
                    if (player.combineNew.itemsCombine.stream().filter(item -> item.isNotNullItem() && item.isDaLinhThach()).count() < 1) {
                        this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Thiếu Đá nguyên Thạch", "Đóng");
                        return;
                    }
            ...
            case NANG_TL_LEN_HUY_DIET:
                if (player.combineNew.itemsCombine.size() == 0) {
                    this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Hãy đưa ta 3 món thần linh, ta sẽ cho 1 món huỷ diệt tương ứng", "Đóng");
                    return;
                }
                if (player.combineNew.itemsCombine.size() == 3) {
                    if (player.combineNew.itemsCombine.stream().filter(item -> item.isNotNullItem() && item.isDTL()).count() != 3) {
...
//                    Item dtl1 = player.combineNew.itemsCombine.get(1);
//                    Item dtl2 = player.combineNew.itemsCombine.get(2);
//                    Item dtl3 = player.combineNew.itemsCombine.get(3);
//                    Item dtl4 = player.combineNew.itemsCombine.get(4);
                    String npcSay = "|2|Con có muốn đổi các món nguyên liệu ?\n|7|"
                    ...
                    if (player.combineNew.itemsCombine.size() > 3) {
                        this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Dư nguyên liệu", "Đóng");
                        return;
                    }
                    this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Còn thiếu nguyên liệu để nâng cấp hãy quay lại sau", "Đóng");
                }
            ...
            case NANG_HUY_DIET_LEN_SKH_VIP:
                if (player.combineNew.itemsCombine.size() == 0) {
                    this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Hãy đưa ta 1 đồ TL - HD - TS", "Đóng");
                    return;
                }
                if (player.combineNew.itemsCombine.size() == 3) {
                    ...
                    if (player.combineNew.itemsCombine.stream().filter(item -> item.isNotNullItem() && item.isDTL()).count() != 1) {
                        this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Thiếu đồ thần", "Đóng");
                        return;
                    }
                    if (player.combineNew.itemsCombine.stream().filter(item -> item.isNotNullItem() && item.isDTS()).count() != 1) {
                        this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Thiếu đồ TS", "Đóng");
                        return;
                    }
...
//                    Item item8 = player.combineNew.itemsCombine.get(7);
//                    Item item9 = player.combineNew.itemsCombine.get(9);
//                    Item item10 = player.combineNew.itemsCombine.get(9);
                    ...
                    this.baHatMit.createOtherMenu(player, ConstNpc.MENU_START_COMBINE,
                            npcSay, "Nâng cấp 5000 ruby", "Từ chối");
                } else {
                    if (player.combineNew.itemsCombine.size() > 3) {
            ...
            case NANG_HUY_DIET_LEN_SKH:
                if (player.combineNew.itemsCombine.size() == 0) {
                    this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Đưa ra 2 món huỷ diệt và nó sẽ thành 1 món kích hoạt", "Đóng");
                    return;
                }
                if (player.combineNew.itemsCombine.size() == 2) {
                    ...
                    if (cailoz != null && doHDKH != null && concak != null && thoivang != null
                        ...
                        for (Item.ItemOption io : cailoz.itemOptions) {
                            if (player.combineNew.goldCombine <= player.inventory.ruby) {
                                ...
                                this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                                        "Cần 1 trang bị thiên sứ kích hoạt,1 trang bị hủy diệt kích hoạt 300000 Thỏi Vàng 2, 300 đá ngũ sắc và 300 thỏi vàng",
                                        "Đóng");
                            }
                        }
                    } else {
                        this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                                "Cần 1 trang bị thiên sứ, 300000 Thỏi Vàng 2, 300 đá ngũ sắc và 300 thỏi vàng",
                                "Đóng");
                    }
                } else {
                    this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU,
        ...
        if (player.combineNew.itemsCombine.size() == 3) {
            ...
            if (keo != null && keo.quantity >= 99 && luoiKiem != null && chuoiKiem != null) {
                if (InventoryServiceNew.gI().getCountEmptyBag(player) > 0) {
                    ...
                    item.itemOptions.add(new Item.ItemOption(50, Util.nextInt(5, 20)));
                    item.itemOptions.add(new Item.ItemOption(77, Util.nextInt(5, 20)));
                    item.itemOptions.add(new Item.ItemOption(103, Util.nextInt(5, 20)));
...
//                    }
    ...
    public void huyDietLenKichHoatThuong(Player player) {
        ...
        Item item1 = player.combineNew.itemsCombine.get(0);
        Item item2 = player.combineNew.itemsCombine.get(1);
        if (item1.isDHD() && item2.isDHD()) {
            if (InventoryServiceNew.gI().getCountEmptyBag(player) > 0) {
                if (player.inventory.ruby < 5000) {
                    Service.getInstance().sendThongBao(player, "Con cần thêm ruby để đổi...");
                    return;
                }
                ...
                if (Util.isTrue(tile, 100)) {
                    ...
                    Service.getInstance().sendThongBao(player, "Trang bị đã bị huỷ diệt");
    ...
    private void doiManhKichHoat(Player player) {
        if (player.combineNew.itemsCombine.size() == 2 || player.combineNew.itemsCombine.size() == 3) {
            ...
            if (nr1s != null && doThan != null) {
                if (InventoryServiceNew.gI().getCountEmptyBag(player) > 0
                        && player.inventory.ruby >= COST_DOI_MANH_KICH_HOAT) {
                    player.inventory.ruby -= COST_DOI_MANH_KICH_HOAT;
    ...
    private void nangCapThanLinhLenHuyDiet(Player player) {
        if (player.combineNew.itemsCombine.size() == 3) {
            ...
            Item dtl1 = player.combineNew.itemsCombine.get(1);
            Item dtl2 = player.combineNew.itemsCombine.get(2);
            if (dtl != null && dtl.isDTL() && dtl1 != null && dtl1.isDTL() && dtl2 != null && dtl2.isDTL()) {
        ...
        if (player.combineNew.itemsCombine.size() == 1) {
            ...
            if (item.quantity < 99) {
                ...
                if (Util.isTrue(80, 20)) {
                    ...
                    Service.gI().sendThongBaoOK(player, "Xin nhé hehe");
                    InventoryServiceNew.gI().subQuantityItemsBag(player, item, 99);
                }
    ...
    public void huyDietLenKichHoatVIP(Player player) {
        ...
        if (player.combineNew.itemsCombine.stream().filter(item -> item.isNotNullItem() && item.isDTL()).count() != 1) {
            Service.gI().sendThongBao(player, "Thiếu đồ thần");
            return;
        }
        if (player.combineNew.itemsCombine.stream().filter(item -> item.isNotNullItem() && item.isDTS()).count() != 1) {
            Service.gI().sendThongBao(player, "Thiếu đồ TS");
            return;
        }
        if (InventoryServiceNew.gI().getCountEmptyBag(player) > 0) {
            if (player.inventory.ruby < 5000) {
            ...
            player.inventory.ruby -= 5000;
            Item dohdodayne = player.combineNew.itemsCombine.stream().filter(Item::isDTS).findFirst().get();
            List<Item> itemdohdlucbovao = player.combineNew.itemsCombine.stream().filter(item -> item.isNotNullItem() && item.isDTS()).collect(Collectors.toList());
    ...
    public void moChiSoCaiTrang(Player player) {
        ...
        if (player.combineNew.itemsCombine.stream().filter(item -> item.isNotNullItem() && item.template.id == 1237)
                .count() != 1) {
        ...
        if (caiTrang != null && longDen != null && longDen.quantity >= 10) {
            ...
            caiTrang.itemOptions.add(new ItemOption(77, Util.nextInt(15, 25)));
            caiTrang.itemOptions.add(new ItemOption(103, Util.nextInt(15, 25)));
            caiTrang.itemOptions.add(new ItemOption(101, Util.nextInt(15, 25)));
            caiTrang.itemOptions.add(new ItemOption(14, Util.nextInt(5, 10)));
            caiTrang.itemOptions.add(new ItemOption(5, Util.nextInt(5, 10)));
            caiTrang.itemOptions.add(new ItemOption(98, Util.nextInt(10, 20)));
            caiTrang.itemOptions.add(new ItemOption(108, Util.nextInt(5, 20)));
            caiTrang.itemOptions.add(new ItemOption(18, Util.nextInt(10, 20)));
    ...
    public void phanRaDoHoiLong(Player player) {
...
//    }
        ...
        if (items.size() != totalItem) {
            Service.gI().sendThongBao(player, "|5|Đồ không khớp");
            return;
        }
...
//                    int tiLe = dtl != null ? 100 : 50;
    ...
    private void doiVeHuyDiet(Player player) {
        if (player.combineNew.itemsCombine.size() == 1) {
            ...
            if (item.isNotNullItem() && item.template.id >= 555 && item.template.id <= 567) {
                if (InventoryServiceNew.gI().getCountEmptyBag(player) > 0
                        && player.inventory.ruby >= COST_DOI_VE_DOI_DO_HUY_DIET) {
                    player.inventory.ruby -= COST_DOI_VE_DOI_DO_HUY_DIET;
    ...
    private void Chuyen_Hoa(Player player) {
        // 1) kiểm tra đúng 2 nguyên liệu
        if (player.combineNew.itemsCombine.size() != 2) {
            baHatMit.createOtherMenu(
        ...
        // 2) tách trangBi và vé
        Item trangBi = null, vesc = null;
        for (Item item : player.combineNew.itemsCombine) {
            if (isTrangBiPhaLeHoa(item)) {
                trangBi = item;
            } else if (isVeSC(item)) {
        ...
        // 3) đọc star và starEmpty (nếu có)
        int star = 0, starEmpty = 0;
        Item.ItemOption io_option = null;
        for (Item.ItemOption io : trangBi.itemOptions) {
            if (io.optionTemplate.id == 102) {
                star = io.param;
            }
        ...
        // 5) tính lại starEmpty để đủ 8
        int needed = (star == 0 ? 8 : 8 - star);
        if (io_option != null) {
            io_option.param = needed;
        } else {
            trangBi.itemOptions.add(new Item.ItemOption(107, needed));
        }

        // 6) tính ngọc cần thiết
        player.combineNew.gemCombine = getGemEpSao(star);

        // 7) hiện menu xác nhận
        String npcSay = trangBi.template.name + "\n|2|"
                + "Cần " + needed + " lỗ pha lê và "
        ...
        // 8) cuối cùng, chỉ cập nhật UI (túi, tiền, giao diện) — nhưng không tiếp tục pha lê tại đây
        InventoryServiceNew.gI().sendItemBags(player);
        Service.gI().sendMoney(player);
        reOpenItemCombine(player);
    }
        ...
        if (InventoryServiceNew.gI().getCountEmptyBag(player) > 0) {
            if (!player.combineNew.itemsCombine.isEmpty()) {
                ...
                if (item != null && item.isNotNullItem() && (item.template.id > 14 && item.template.id <= 20)) {
                    ...
                    // THÊM: nếu có >= 7^4 = 2401 viên, nhận 7^3 = 343 viên cấp cao hơn
                    if (soLuong >= 2401) {
                        for (int i = 0; i < 343; i++) {
                            Item nr = ItemService.gI().createNewItem((short) (item.template.id - 1));
                            InventoryServiceNew.gI().addItemBag(player, nr);
                        }
                        InventoryServiceNew.gI().subQuantityItemsBag(player, item, 2401);
                    } else if (soLuong >= 343) { // 7^3
                        for (int i = 0; i < 49; i++) {
                            Item nr = ItemService.gI().createNewItem((short) (item.template.id - 1));
                            InventoryServiceNew.gI().addItemBag(player, nr);
                        }
                        InventoryServiceNew.gI().subQuantityItemsBag(player, item, 343);
                    } else if (soLuong >= 49) { // 7^2
                        for (int i = 0; i < 7; i++) {
                            Item nr = ItemService.gI().createNewItem((short) (item.template.id - 1));
                            InventoryServiceNew.gI().addItemBag(player, nr);
                        }
                        InventoryServiceNew.gI().subQuantityItemsBag(player, item, 49);
                    } else if (soLuong >= 7) { // 7^1
                        Item nr = ItemService.gI().createNewItem((short) (item.template.id - 1));
                        InventoryServiceNew.gI().addItemBag(player, nr);
    ...
    private void epantrangbi(Player player) {
        if (player.combineNew.itemsCombine.size() == 4) {
            ...
            if (player.inventory.ruby < gold) {
                Service.gI().sendThongBao(player, "Không đủ Thỏi Vàng 2 để thực hiện");
                return;
            }
        ...
        if (player.combineNew.itemsCombine.size() == 3) {
            ...
            if (quatrung != null && honlinhthu != null && thoivang.quantity >= 20 && honlinhthu.quantity >= 99) {
                ...
                if (player.thienThanEgg == null) {
                    ThienThanEgg.createThienThanEgg(player);
                }
        ...
        if (player.combineNew.itemsCombine.size() == 3) {
            ...
            if (quatrung != null && honlinhthu != null && thoivang.quantity >= 20 && honlinhthu.quantity >= 99) {
                ...
                if (player.hacAmEgg == null) {
                    HacAmEgg.createHacAmEgg(player);
                }
                ChangeMapService.gI().changeMapInYard(player,
                        player.zone.map.mapId, player.zone.zoneId, player.location.x);
            } else {
    ...
    private boolean isVeSC(Item item) {
        return item != null && (item.template.type == 27) && (item.template.id == 2139);
    }
...
//                return "vào hành trang\nChọn 3 trang bị hủy diệt khác nhau " + "có tỉ lệ ra đồ kích hoạt hủy diệt" + "\nsau đó chọn 'Nâng cấp'\n"
//                        + "Lưu ý món đầu tiên đặt vào sẽ là trang bị để nâng cấp!";
//            case NANG_CAP_SKH_TS:
//                return "vào hành trang\nChọn 3 trang bị thiên sứ khác nhau " + "có tỉ lệ ra đồ kích hoạt thiên sứ" + "\nsau đó chọn 'Nâng cấp'\n"
//                        + "Lưu ý món đầu tiên đặt vào sẽ là trang bị để nâng cấp!";
            case NANG_CAP_DO_HD:
                return "vào hành trang\nChọn 3 trang bị thần linh bất kì\nkèm 1 đá Linh Thạch(mua ở tiên cá) \n "
                        + "sẽ cho ra đồ huy diet từ 0-15% chỉ số\nđồ sẽ theo món bỏ vào số 1"
                        + "Sau đó chọn 'Nâng Cấp'";
            case CHE_TAO_TRANG_BI_TS:
            ...
            case NANG_TL_LEN_HUY_DIET:
                return "Vào hành trang\n chọn 3 món thần linh bất kỳ, sau đó nhấn 'Nâng cấp'";

            case NANG_HUY_DIET_LEN_SKH_VIP:
                return "Vào hành trang\n Chọn 3 món HD - TL - TS bất kỳ, món đầu tiên sẽ làm gốc, sau đó chọn 'Nâng cấp'";
            case NANG_HUY_DIET_LEN_SKH:
                return "Vào hành trang\n Chọn 2 món huỷ diệt bất kỳ, sau đó chọn 'Nâng câp'";
            case MO_CHI_SO_BONG_TAI_CAP3:
