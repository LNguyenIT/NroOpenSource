...
public class IntrinsicService {
    ...
    private static final int[] COST_OPEN = { 50, 100, 150, 300, 600 };
    ...
    public void sattdhd(Player player) {
        NpcService.gI().createMenuConMeo(player, ConstNpc.menutdhd, -1,
                "chọn lẹ đi để tau đi chơi với ny", "Set\nThên Xin Hăng", "Set\nGenki", "Set\nKame", "Từ chối");

    }

    public void satnmhd(Player player) {
        NpcService.gI().createMenuConMeo(player, ConstNpc.menunmhd, -1,
                "chọn lẹ đi để tau đi chơi với ny", "Set\nKI", "Set\nLiên Hoàn", "Set\nChim", "Từ chối");

    }

    public void setxdhd(Player player) {
        NpcService.gI().createMenuConMeo(player, ConstNpc.menuxdhd, -1,
                "chọn lẹ đi để tau đi chơi với ny", "Set\nGalick", "Set\nKhỉ", "Set\nHP", "Từ chối");
    ...
    public void open(Player player) {
        if (player.nPoint.power >= 15000000000L) {
            ...
            Service.gI().sendThongBao(player, "Yêu cầu sức mạnh tối thiểu 15 tỉ");
        }
    }

    public void openVip(Player player) {
        if (player.nPoint.power >= 1500000000) {
            int rubyRequire = 100;
            if (player.inventory.ruby >= 100) {
                ...
                Service.gI().sendThongBao(player, "Bạn không có đủ ngọc, còn thiếu "
                        + (rubyRequire - player.inventory.ruby) + " ngọc nữa");
            }
        } else {
            Service.gI().sendThongBao(player, "Yêu cầu sức mạnh tối thiểu 15 tỉ");
        }
