...
import red.team.matches.PVP;
import red.team.matches.PVPManager;
...
import static red.services.func.SummonDragon.DRAGON_SHENRON;
...
public class Service {
    ...
    public void showListTop(Player player, List<TOP> tops) {
        ...
        try {
            ...
            e.printStackTrace();
            Logger.logException(Manager.class, e, "Lỗi show list top");
        }
    }
    ...
    public void sendchienlinh(Player player, short smallId) {
        ...
        try {
            ...
            e.printStackTrace();
            Logger.logException(Manager.class, e, "Lỗi send chien linh");
        }
    }
    ...
    public void sendchienlinh(Player me, Player pl) {
        ...
        try {
            ...
            e.printStackTrace();
            Logger.logException(Manager.class, e, "Lỗi send chienlinh");
        }
    }
    ...
    public void sendTitleRv(Player player, Player p2, int part) {
        Message me;
        try {
            me = new Message(-128);
            me.writer().writeByte(0);
    ...
    public void chat(Player player, String text) {
        ...
        if (player.getSession() != null && player.isAdmin()) {
            ...
            if (text.equals("client")) {
                ...
                try {
                    ...
                    e.printStackTrace();
                    Logger.logException(Manager.class, e, "command service");
                }
            }
            if (text.equals("nrnm")) {
            ...
            if (text.equals("ad")) {
                NpcService.gI().createMenuConMeo(player, ConstNpc.MENU_ADMIN, -1,
                        ...
                        "Ngọc rồng", "Đệ tử", "Bảo trì", "Tìm kiếm\nngười chơi", "Boss", "Giftcode", "Nạp tiền",
                        "Thông báo Server", "Thông báo riêng tư", "Tặng quà mốc nạp", "Đóng");
                return;
        ...
        // null));
        // } else if (text.equals("billegg")) {
        // BillEgg.createBillEgg(player);
        // sendThongBao(player, "Khởi Tạo bill Thành Công: " + (player.billEgg !=
        // null));
        // }else if (text.equals("ngokhongegg")) {
        // NgokhongEgg.createNgokhongEgg(player);
        // sendThongBao(player, "Khởi Tạo ngokhong Thành Công: " + (player.ngokhongEgg
        // != null));
        // }else if (text.equals("gokuegg")) {
        // GokuEgg.createGokuEgg(player);
        // sendThongBao(player, "Khởi Tạo goku Thành Công: " + (player.gokuEgg !=
        // null));
        // }
    ...
    public String getCurrStrLevel(Player pl) {
        ...
        } else if (sucmanh < 18000000000000L) {
            return "Gay Lỏd";
        }
...
//    }\
    ...
    public void sendThongBaoFromAdmin(Player player, String text) {
        sendBigMessage(player, 7849, text);
    }
    ...
    public void sendFootRv(Player player, Player p2, int part) {
        Message me;
        try {
            me = new Message(-128);
            me.writer().writeByte(0);
    ...
    public void chooseFlag(Player pl, int index) {
        if (MapService.gI().isMapBlackBallWar(pl.zone.map.mapId) || MapService.gI().isMapMaBu(pl.zone.map.mapId)
                || MapService.gI().isMapMabu13h(pl.zone.map.mapId) || MapService.gI().isMapSatan(pl.zone.map.mapId)
                || MapService.gI().isMapPVP(pl.zone.map.mapId) || MapService.gI().isMapVodai(pl.zone.map.mapId)) {
                ...
                for (int i = 0; i < sizeSkill; i++) {
                    if (pl.pet.playerSkill.skills.get(i).skillId != -1) {
                        ...
                        switch (i) {
                            ...
                            case 4:
                                msg.writer().writeShort(-1);
                                msg.writer().writeUTF("Cần đạt sức mạnh 80tỷ\nđể mở");
                                break;
                            case 5:
                                msg.writer().writeShort(-1);
                            ...
                            default:
                                msg.writer().writeShort(-1);
                                msg.writer().writeUTF("Cần đạt sức mạnh 200tỷ\nđể mở");
                                break;
                        }
                    }
                }
