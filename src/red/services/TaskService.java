...
public class TaskService {
    ...
    private static red.services.TaskService i;

    public static red.services.TaskService gI() {
        if (i == null) {
            i = new red.services.TaskService();
        }
        return i;
    }
    ...
    public boolean checkDoneTaskTalkNpc(Player player, Npc npc) {
        switch (npc.tempId) {
            ...
            case ConstNpc.JACO:
                return doneTask(player, ConstTask.TASK_35_2);
            ...
            case ConstNpc.GOKU_SSJ:
                return (doneTask(player, ConstTask.TASK_34_3)
...
//        }
        ...
        if (!player.isBoss && !player.isPet && item != null) {
            switch (item.itemTemplate.id) {
                ...
                case 725: // em bé
                    ...
                    break;
                case 1443:
                    if (player.isPl() && item.itemTemplate.id == 1443) {
        ...
        if (player != null && !player.isBoss && !player.isPet) {
            switch ((int) boss.id) {
                ...
                case BossID.CADIC_BLUE:
                    doneTask(player, ConstTask.TASK_35_1);
                    break;
                ...
                case BossID.FROST:
                    switch (boss.currentLevel) {
                        case 0:
                            ...
                            break;
                        case 1:
                            doneTask(player, ConstTask.TASK_39_1);
                            break;
                        case 2:
                ...
                case BossID.SUPER_ANDROID_17:
                    doneTask(player, ConstTask.TASK_40_2);
                    break;
                ...
                case BossID.THAN_HUY_DIET_CHAMPA:
                    doneTask(player, ConstTask.TASK_47_0);
                    break;
                ...
                case BossID.ZENO:
                    switch (boss.currentLevel) {
                        case 0:
                            ...
                            break;
                        case 1:
                            doneTask(player, ConstTask.TASK_48_1);
                            break;
                        case 2:
                            doneTask(player, ConstTask.TASK_48_2);
                            break;
                    }
                    ;
                    break;
                ...
                case BossID.MABU_THAN:
                    doneTask(player, ConstTask.TASK_49_1);
                    break;
        ...
        if (!player.isBoss && !player.isPet) {
            switch (mob.tempId) {
                case ConstMob.MOC_NHAN:
                    ...
                    doneTask(player, ConstTask.TASK_50_0);
                    break;
        ...
        if (TaskService.gI().isCurrentTask(player, idTaskCustom)) {
            ...
            switch (idTaskCustom) {
                ...
                case ConstTask.TASK_3_2:
                    try {
                        InventoryServiceNew.gI().subQuantityItemsBag(player,
                                InventoryServiceNew.gI().findItemBag(player, 78), 1);
                    } catch (Exception ex) {
                    }
                    InventoryServiceNew.gI().sendItemBags(player);
                    Service.gI().sendFlagBag(player);
                ...
                case ConstTask.TASK_34_3:
                    npcSay(player, ConstNpc.GOKU_SSJ,
                            "Ngọc Rồng online");
                    break;
    ...
    private void rewardDoneTask(Player player) {
        ...
        if (player.playerTask.taskMain.id > 0 && player.playerTask.taskMain.id < 25) {
            ...
            player.inventory.ruby += (player.playerTask.taskMain.id < 5 && player.playerTask.taskMain.id >= 0)
                    ? 100 * (player.playerTask.taskMain.id + 1)
                    : 500;
    ...
    public void paySideTask(Player player) {
        if (player.playerTask.sideTask.template != null) {
            if (player.playerTask.sideTask.isDone()) {
                ...
                player.nPoint.powerUp(goldReward);
                if (Util.isTrue(100, 100)) {
                    Service.gI().sendMoney(player);
                    player.inventory.addTV(player,goldReward);
                    Service.gI().sendThongBao(player, "Mày Nhận Được " + goldReward + " Vàng");
                }
                Service.gI().sendThongBao(player, "Mày Nhận Được " + goldReward + " Sức Mạnh");
                player.playerTask.sideTask.reset();
            } else {
    ...
    public void checkDoneSideTaskKillMob(Player player, Mob mob) {
        if (player.playerTask.sideTask.template != null) {
            if ((player.playerTask.sideTask.template.id == 0 && mob.tempId == ConstMob.KHUNG_LONG)
                    ...
                    || (player.playerTask.sideTask.template.id == 58 && mob.tempId == ConstMob.TOBI)
                    || (player.playerTask.sideTask.template.id == 59 && mob.tempId == ConstMob.CABIRA)
                    || (player.playerTask.sideTask.template.id == 60 && mob.tempId == ConstMob.MOC_NHAN)) {
    ...
    public void changeClanTask(Player player, byte level) {
        if (player.playerTask.clanTask.leftTask > 0) {
            ...
            Service.gI().sendThongBao(player, "Bạn nhận được nhiệm vụ: " + player.playerTask.clanTask.getName());
        } else {
            Service.gI().sendThongBao(player,
                    "Bạn đã nhận hết nhiệm vụ hôm nay. Hãy chờ tới ngày mai rồi nhận tiếp");
        }
    ...
    public void removeClanTask(Player player) {
        Service.gI().sendThongBao(player, "Bạn vừa hủy bỏ nhiệm vụ " + player.playerTask.clanTask.getName());
        player.playerTask.clanTask.reset();
    }
    ...
    public void payClanTask(Player player) {
        if (player.playerTask.clanTask.template != null) {
            if (player.playerTask.clanTask.isDone()) {
                int goldReward = 0;
                switch (player.playerTask.clanTask.level) {
                    case ConstTask.EASY:
                        goldReward = 1;
                        break;
                    case ConstTask.NORMAL:
                ...
                Item capsuleBang = ItemService.gI().createNewItem((short) 1427, goldReward);
                InventoryServiceNew.gI().addItemBag(player, capsuleBang);
                InventoryServiceNew.gI().sendItemBags(player);
                Service.gI().sendThongBao(player, "Bạn nhận được "+goldReward+" capsule bang !");
                player.playerTask.clanTask.reset();
            } else {
                Service.gI().sendThongBao(player, "Bạn chưa hoàn thành nhiệm vụ");
            }
    ...
    public void checkDoneClanTaskKillMob(Player player, Mob mob) {
        if (player.playerTask.clanTask.template != null) {
            if ((player.playerTask.clanTask.template.id == 0 && mob.tempId == ConstMob.MOC_NHAN) ||
    ...
    private void notifyProcessClanTask(Player player) {
        ...
        if (percentDone != 100) {
            ...
            Service.gI().sendThongBao(player, "Chúc mừng bạn đã hoàn thành nhiệm vụ, "
                    + "bây giờ hãy quay về Ngưu ma vương trả nhiệm vụ.");
        }
    }
}
