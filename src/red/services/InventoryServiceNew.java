...
import red.MaQuaTang.MaQuaTang;
...
public class InventoryServiceNew {
    ...
    public void addItemGiftCodeToPlayer(Player p, MaQuaTang giftcode) {
        ...
        for (Integer key : keySet) {
            ...
            if (idItem == -1) {
                p.inventory.gold = Math.min(p.inventory.gold + (long) quantity, 20000000000L);
...
//                Service.gI().sendTitle(player, item.template.part);
    ...
    public void itemBagToPetBody(Player player, int index) {
        ...
        if (player.pet != null && player.pet.nPoint.power >= 1500000 ) {
            ...
            Service.getInstance().sendThongBaoOK(player, "Đệ Cần Đạt 1tr5 Sức Mạnh Và Trang Bị Phải Phù Hợp");
        }
    }
    ...
    private boolean addItemSpecial(Player player, Item item) {
        ...
        switch (item.template.id) {
            ...
            case 74: //đùi gà nướng
                ...
                return true;
            case 1427: //capsule bang
                player.clan.capsuleClan+=item.quantity;
                player.clanMember.clanPoint+=item.quantity;
