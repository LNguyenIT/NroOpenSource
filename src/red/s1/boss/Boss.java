...
import red.team.boss.iboss.IBossNew;
import red.team.boss.iboss.IBossOutfit;
...
public class Boss extends Player implements IBossNew, IBossOutfit {
    ...
    public Player getPlayerAttack() {
        if (this.zone != null && this.playerTarger != null && (this.playerTarger.isDie() || !this.zone.equals(this.playerTarger.zone)
                || this.playerTarger.effectSkill.isAnThan || this.playerTarger.isNewPet)) {
    ...
    public void die(Player plKill) {
        if (plKill != null) {
            reward(plKill);
            ServerNotify.gI().notify(plKill.name + " vừa tiêu diệt được " + this.name + ", hên vcl!");
        }
    ...
    public void wakeupAnotherBossWhenAppear() {
        System.out.println(this.name + ":" + this.zone.map.mapName + " khu vực " + this.zone.zoneId + "("
                + this.zone.map.mapId + ")");
        if (
                   MapService.gI().isMapMiNuong(this.zone.map.mapId) 
                && MapService.gI().isMapMabu13h(this.zone.map.mapId)
                && MapService.gI().isMapKhiGaHuyDiet(this.zone.map.mapId)
                && MapService.gI().isMapMaBu(this.zone.map.mapId) 
                 && MapService.gI().isMapVodai(this.zone.map.mapId) 
                && MapService.gI().isMapDoanhTrai(this.zone.map.mapId)
                && MapService.gI().isMapBanDoKhoBau(this.zone.map.mapId)
                && MapService.gI().isMapSatan(this.zone.map.mapId)
                && MapService.gI().isMapDiaNguc(this.zone.map.mapId)
                && MapService.gI().isMapBlackBallWar(this.zone.map.mapId)) {
            System.out.println("BOSS " + this.name + " đã xuất hiện tại " + this.zone.map.mapName);
        }
        if (this.bossAppearTogether == null || this.bossAppearTogether[this.currentLevel] == null) {
            return;
        }
