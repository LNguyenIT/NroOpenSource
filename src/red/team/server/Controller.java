...
import red.services.ChatGlobalService;
import red.services.ItemTimeService;
import red.services.SubMenuService;
...
public class Controller implements IMessageHandler {
    ...
    public void onMessage(ISession s, Message _msg) {
        ...
        try {
            ...
            e.printStackTrace();
            Logger.logException(Manager.class, e, "Lỗi cmd controler");
        } finally {
            _msg.cleanup();
            _msg.dispose();
        }
...
//                        }
    ...
    public void sendInfo(MySession session) {
        ...
        if (player.inventory.itemsBody.get(12).isNotNullItem()) {
           new Thread(() -> {
                try {
                    Thread.sleep(1000);
                    Service.getInstance().sendTitle(player, player.inventory.itemsBody.get(12).template.part);
                } catch (Exception e) {
                }
            }).start();
        }
        if (player.inventory.itemsBody.get(13).isNotNullItem()) {
           new Thread(() -> {
                try {
                    Thread.sleep(1000);
                    Service.getInstance().sendTitle(player, player.inventory.itemsBody.get(13).template.part);
                } catch (Exception e) {
                }
            }).start();
        }
        if (player.inventory.itemsBody.get(14).isNotNullItem()) {
           new Thread(() -> {
                try {
                    Thread.sleep(1000);
                    Service.getInstance().sendTitle(player, player.inventory.itemsBody.get(14).template.part);
                } catch (Exception e) {
                }
            }).start();
        }
        if (player.inventory.itemsBody.get(15).isNotNullItem()) {
           new Thread(() -> {
                try {
                    Thread.sleep(1000);
                    Service.getInstance().sendTitle(player, player.inventory.itemsBody.get(15).template.part);
                } catch (Exception e) {
                }
            }).start();
        }
        ...
        if (player.inventory.ruby >= 1500000000) {
            new Thread(() -> {
                try {
                    Thread.sleep(10000);
        ...
        if (player.nPoint.hpMax > 1500000000 && !player.isAdmin()) {
            new Thread(() -> {
                try {
                    Thread.sleep(10000);
                    GirlkunDB.executeUpdate("update account set ban = 1 where id = ? and username = ?",
        ...
        if (player.nPoint.mpMax > 1500000000 && !player.isAdmin()) {
            new Thread(() -> {
                try {
                    Thread.sleep(10000);
                    GirlkunDB.executeUpdate("update account set ban = 1 where id = ? and username = ?",
        ...
        if ((player.nPoint.mpg > 1100000 || player.nPoint.hpg > 1100000 || player.nPoint.dameg > 34000)
                && !player.isAdmin()) {
            new Thread(() -> {
                try {
                    Thread.sleep(10000);
    ...
    private void sendThongBaoServer(Player player) {
        Service.getInstance().sendThongBaoFromAdmin(player, "|1|Chào mừng bạn " + player.name + " đến với Ngọc Rồng online\n"
