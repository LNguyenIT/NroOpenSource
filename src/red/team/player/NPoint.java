...
public class NPoint {
    ...
    private void setPointWhenWearClothes() {
        ...
        if (this.player.capChuyenSinh > 0 && this.player.capChuyenSinh < 11) {
            tlDame.add(2 * this.player.capChuyenSinh);
            tlHp.add(2 * this.player.capChuyenSinh);
            tlMp.add(2 * this.player.capChuyenSinh);
        }
...
//                Service.gI().sendThongBaoOK(player, "Vui lÃ²ng má»Ÿ giá»›i háº¡n sá»©c máº¡nh");
        ...
        if (type == 3) {
            int defcur = this.defg;
            long total = 0;

            for (int i = 0; i < 50; i++) {
                long tiemNangUse1 = (defcur + 5) * 100000; // Cập nhật dựa trên defcur
                total += tiemNangUse1;
                defcur++;
            }

            if ((this.defg + 50) <= getDefLimit()) {
                if (doUseTiemNang(total)) {
                    defg += 50;
                }
            } else if ((this.defg + point) <= getDefLimit()) {
                long tiemNangUse1 = (this.defg + 5) * 100000; // Tính lại tiemNangUse cho từng point
                if (doUseTiemNang(tiemNangUse)) {
                    defg += point;
                }
            } else {
    ...
    private void setHpMax() {
        ...
        if (this.player.setClothes.nappa == 5) {
            this.hpMax += ((long) this.hpMax * 10 / 10);
        }
        // set worldcup
...
//        Service.gI().sendThongBao(this.player,"Thứ tự đệ mày là "+((Pet) this.player).typePet);
        ...
        if (this.player.isPet && ((Pet) this.player).typePet == 4
                && ((Pet) this.player).master.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA4) {
            this.hpMax += ((long) this.hpMax * 60 / 100);
        }
        // pet ngokhong
...
//        }
        ...
        if (this.player.effectFlagBag.useMeoMun) {
            this.hpMax += ((long) this.hpMax * 15 / 100);
                ...
                // keo 1
        if (this.player.itemTime != null && this.player.itemTime.isCand1) {
    ...
    private void setMpMax() {
        ...
        if (this.player.isPet && ((Pet) this.player).typePet == 5
                && ((Pet) this.player).master.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA) {
            this.hpMax += ((long) this.mpMax * 60 / 100);
        }
        if (this.player.isPet && ((Pet) this.player).typePet == 5
                && ((Pet) this.player).master.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA2) {
            this.hpMax += ((long) this.mpMax * 60 / 100);
        ...
        if (this.player.itemTime != null && this.player.itemTime.isCand2) {
            this.mpMax += this.mpMax / 10;
...
//            }
            ...
            if (this.player.isPet) {
                if (this.player.itemTime.isCand4 ) {
