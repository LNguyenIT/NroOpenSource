 ...
 */
package red.team.boss.list_boss.sontinhthuytinh;
...
public class Thuytinh extends Boss {
    ...
    public Thuytinh(int dame, int hp, Zone zone) throws Exception {
        super(Util.randomBossId(), Thuytinh.THUY_TINH);
        this.dameClan = dame;
        this.hpClan = hp;
        this.zoneFinal = zone;
    }

    public Thuytinh(int dame, int hp, Zone zone, int id, BossData... data) throws Exception {
        super(id, data);
        this.dameClan = dame;
        this.hpClan = hp;
        this.zoneFinal = zone;
    ...
    private static final BossData THUY_TINH = new BossData(
            "Thủy tinh", //name
...
//            ChangeMapService.gI().changeMap(this, zoneFinal, 897, 384);
    ...
    public void moveTo(int x, int y) {
        if (this.location.x >= 300 || this.location.x <= 600) {
