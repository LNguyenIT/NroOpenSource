...
public class Adminthan extends Boss {
    ...
    public Adminthan() throws Exception {
        super(BossID.ADMIN_THAN, BossesData1.ADMIN_THAN);
    }

    @Override
    public void reward(Player plKill) {
        for (int i = 0; i < 1; i++) {
            ItemMap it = new ItemMap(this.zone, 2121, 1, this.location.x + i * 20,
                    this.zone.map.yPhysicInTop(this.location.x,
                            this.location.y - 24),
                    -1);
            Service.gI().dropItemMap(this.zone, it);
        }if(plKill.pet.typePet > 2  &&  Util.isTrue(25, 100)){
             ItemMap it = new ItemMap(this.zone, 2121, 1, this.location.x, this.zone.map.yPhysicInTop(this.location.x,
