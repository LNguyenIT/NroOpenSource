...
import red.services.func.Trade;
...
public class ShopServiceNew {
    ...
    private void openShopType4(Player player, String tagName, List<Item> items) {
        ...
        try {
            ...
            e.printStackTrace();
            Logger.logException(Manager.class, e, "Lỗi open shop");
        }
    }
        ...
        switch (itS.template.id) {
            ...
            case 861:
                if (pl.inventory.ruby >= buySpec) {
                    ...
                    Service.gI().sendThongBao(pl, "Bạn không có đủ hồng ngọc");
                    isBuy = false;
                }
                break;
                case 533:
                //533 kẹo giáng sinh
                Item vp = InventoryServiceNew.gI().findItem(pl.inventory.itemsBag, 533);
                if (vp != null) {
                    if (vp.quantity >= 100 && InventoryServiceNew.gI().getCountEmptyBag(pl) > 0) {
                        InventoryServiceNew.gI().subQuantityItemsBag(pl, vp, 100);
                        isBuy = true;
                    } else {
                        InventoryServiceNew.gI().subQuantityItemsBag(pl, vp, 100);
                        Service.gI().sendThongBao(pl, "Đủ đâu mà đổi");
                    }

                } else {
                    Service.gI().sendThongBao(pl, "Có gì đâu mà đòi hỏi");
                }
                break;
            default:
